...
// Telegram Bot + Mini App — ưu tiên ENV > DB (phục vụ deploy, token thật nằm ở ENV)
if(!db.settings.telegram) db.settings.telegram = { ...DEFAULT_SETTINGS.telegram };
          ...
          for(let r=1;r<vals.length;r++){
            ...
            if(!app){
              const newApp = {
                ...
                result: (row[ci('Kết quả', 15)]||'').toString().trim()||'Đạt vòng hồ sơ',
              ...
              // Bắn thông báo thời gian thực cho Bot Quản trị HR (@umbomilkhrbot) theo quy tắc Không Miss & Chống Trùng
              try {
                const notifText = formatNewApplicantTelegramAlert(newApp);
                notifyHRMaster('applicant', notifText, null, { dedupeKey: 'applicant:' + id, applicantId: id }).catch(()=>{});
              } catch(e) {}
            } else {
...
// Định dạng thông báo ứng viên/nhân viên mới đăng ký từ Sheet NHAN_VIEN_MOI cho Bot Quản trị
function formatNewApplicantTelegramAlert(app){
  const id = app.id || ('UV_' + Date.now());
  const createdDate = app.createdAt ? (fmtDMY(String(app.createdAt).split('T')[0]) + ' ' + (String(app.createdAt).split('T')[1]||'').slice(0, 8)).trim() : getVietnamDateTimeStr();
  const name = app.name || 'Chưa rõ';
  const gender = app.gender || '—';
  const birthYear = app.birthYear || '—';
  const education = app.education || '—';
  const hometown = app.hometown || '—';
  const phone = app.phone || '—';
  const shift = app.shiftText || (DEFAULT_SHIFTS[app.shiftPreference]?.name || app.shiftPreference) || '—';
  const branch = app.branchText || app.branchPreference || '—';
  const exp = app.experience || 'Chưa có';
  const handling = app.handling || '—';
  const fb = app.facebook || '—';
  const src = app.source || 'Form Tuyển Dụng';
  const score = app.aiScore !== null && app.aiScore !== undefined ? app.aiScore + ' / 10' : 'Đang chấm';
  const res = app.result || (app.status === 'NEW_APPLICANT' ? 'Đạt vòng hồ sơ' : app.status || 'Đạt vòng hồ sơ');

  return `🆕 <b>THÔNG BÁO: CÓ ỨNG VIÊN / NHÂN VIÊN MỚI ĐĂNG KÝ!</b>\n`
    + `📊 <b>Nguồn:</b> Google Sheet <code>NHAN_VIEN_MOI</code> (Form Tuyển Dụng)\n\n`
    + `━━━━━━━━━━━━━━━━━━━━━━━━━━\n`
    + `🆔 <b>Mã ứng viên:</b> <code>${id}</code>\n`
    + `📅 <b>Ngày đăng ký:</b> ${createdDate}\n`
    + `👤 <b>Họ và tên:</b> ${name}\n`
    + `🚻 <b>Giới tính:</b> ${gender}  |  🎂 <b>Năm sinh:</b> ${birthYear}\n`
    + `🎓 <b>Trình độ:</b> ${education}\n`
    + `🏡 <b>Quê quán:</b> ${hometown}\n`
    + `📱 <b>Số điện thoại:</b> <code>${phone}</code>\n`
    + `⏰ <b>Ca đăng ký:</b> ${shift}\n`
    + `🏪 <b>Chi nhánh ĐK:</b> ${branch}\n`
    + `💼 <b>Kinh nghiệm:</b> ${exp}\n`
    + `⚡ <b>Xử lý đột xuất:</b> ${handling}\n`
    + `🌐 <b>Facebook:</b> ${fb}\n`
    + `📣 <b>Nguồn biết tin:</b> ${src}\n`
    + `🤖 <b>Điểm AI đánh giá:</b> <b>${score}</b>\n`
    + `🎯 <b>Kết quả sơ tuyển:</b> ${res} ✅\n`
    + `━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`
...
function notifyAdminAndHR({ action, employeeId, employeeName, branchId, title, message, type='info', data=null }){
  ...
  if(db.notifications.length >= 500) db.notifications = db.notifications.slice(0, 499);
...
function checkOffConflict(branchId, shift, date, excludeEmployeeId, isTestRequester){
  ...
  return db.offRequests.find(r=> {
    ...
    if(!emp) return false;
    const isTestTarget = emp.isTest || isTestRecord(emp);
    // If requester is test, only conflict with other test employees (not real employees)
    // If requester is real, conflict with both real and test employees
    if(isTestRequester && !isTestTarget) return false; // Test requester doesn't conflict with real employees
    if(!isTestRequester && isTestTarget) return false; // Real requester doesn't conflict with test employees (they're cleaned up)
...
app.post('/api/recruitment/form-submit', async (req,res)=>{
  ...
  // Bắn thông báo thời gian thực cho Bot Quản trị HR (@umbomilkhrbot) theo quy tắc Không Miss & Chống Trùng
  try {
    const notifText = formatNewApplicantTelegramAlert(applicant);
    notifyHRMaster('applicant', notifText, null, { dedupeKey: 'applicant:' + applicant.id, applicantId: applicant.id }).catch(()=>{});
  } catch(e) {}
...
async function sendZaloBotNotification(payload) {
  ...
  saveDB();
  io.emit('zalo:update', db.zaloRecords);
  // Kênh bổ sung: đẩy song song qua Telegram nếu NV đã liên kết (không ảnh hưởng Zalo)
  try{ notifyTelegramForPhone(phone, content); }catch(e){}
  return zr;
}
...
// ============ DRIVE AUTO-BACKUP db.json (kho backup cấp 2 trên Google Drive 2TB) ============
// Chạy mỗi đêm 02:00 giờ VN + nút bấm tay. Giữ 30 bản mới nhất (umb-db-*.json).
let __lastDriveBackupDay = '';
async function backupDbToDrive(reason) {
  if (OUTBOUND_SYNC_DISABLED) return { ok: false, skipped: 'disabled', reason: 'Chế độ test — không đẩy ra ngoài' };
  try {
    const token = await getGoogleAccessToken();
    if (!token) return { ok: false, skipped: 'no-token', reason: 'Chưa cấu hình Service Account (Cài đặt → Google Sheet)' };
    const folderId = db.settings?.googleDrive?.backupFolderId || db.settings?.googleDrive?.rootFolderId;
    if (!folderId) return { ok: false, skipped: 'no-folder', reason: 'Chưa cấu hình Drive Folder (Cài đặt → Google Drive)' };
    if (!fs.existsSync(DATA_FILE)) return { ok: false, skipped: 'no-file', reason: 'Không thấy data/db.json' };
    const vn = new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Ho_Chi_Minh' }));
    const pad = (n) => String(n).padStart(2, '0');
    const name = `umb-db-${vn.getFullYear()}${pad(vn.getMonth() + 1)}${pad(vn.getDate())}-${pad(vn.getHours())}${pad(vn.getMinutes())}.json`;
    const data = fs.readFileSync(DATA_FILE);
    const boundary = '-------umbdbbackup31415926';
    const metadata = JSON.stringify({ name, parents: [folderId], mimeType: 'application/json' });
    const part1 = `--${boundary}\r\nContent-Type: application/json; charset=UTF-8\r\n\r\n${metadata}\r\n`;
    const part2Header = `--${boundary}\r\nContent-Type: application/json\r\n\r\n`;
    const end = `\r\n--${boundary}--`;
    const bodyBuffer = Buffer.concat([Buffer.from(part1), Buffer.from(part2Header), data, Buffer.from(end)]);
    const upRes = await fetch('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart', {
      method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': `multipart/related; boundary=${boundary}` }, body: bodyBuffer
    });
    const upData = await upRes.json().catch(() => ({}));
    if (!upData.id) return { ok: false, skipped: 'upload-failed', reason: upData?.error?.message || 'Drive từ chối upload' };
    const url = `https://drive.google.com/file/d/${upData.id}/view`;
    if (!db.driveFiles) db.driveFiles = [];
    db.driveFiles.unshift({ id: uuidv4(), employeeId: '', name: '', date: getVietnamTodayStr(), type: 'DB_BACKUP', fileName: name, driveFileId: upData.id, url, size: data.length, reason: reason || 'AUTO', createdAt: getVietnamISOString(), sync_status: 'SYNCED' });
    if (db.driveFiles.length > 300) db.driveFiles.length = 300;
    saveDB();
    try { io.emit('drive:update', db.driveFiles.slice(0, 20)); } catch (e) {}
    // Tỉa bản cũ trên Drive: giữ 30 bản umb-db-* mới nhất
    try {
      const q = encodeURIComponent(`'${folderId}' in parents and name contains 'umb-db-' and trashed=false`);
      const lr = await fetch(`https://www.googleapis.com/drive/v3/files?q=${q}&orderBy=createdTime desc&pageSize=100&fields=files(id,createdTime)`, { headers: { Authorization: `Bearer ${token}` } });
      const lj = await lr.json().catch(() => ({}));
      const olds = (lj.files || []).slice(30);
      for (const f of olds) { try { await fetch(`https://www.googleapis.com/drive/v3/files/${f.id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } }); } catch (e) {} }
    } catch (e) { console.error('[DRIVE BACKUP] prune error', e.message); }
    console.log(`[DRIVE BACKUP] OK ${name} (${Math.round(data.length / 1024)}KB) -> ${upData.id} [${reason || 'AUTO'}]`);
    return { ok: true, fileId: upData.id, name, size: data.length, url };
  } catch (e) {
    console.error('[DRIVE BACKUP] error', e.message);
    return { ok: false, skipped: 'error', reason: e.message };
  }
}
// Lịch đêm 02:00 giờ VN — kiểm tra mỗi phút
setInterval(() => {
  try {
    const vn = new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Ho_Chi_Minh' }));
    const pad = (n) => String(n).padStart(2, '0');
    const day = `${vn.getFullYear()}-${pad(vn.getMonth() + 1)}-${pad(vn.getDate())}`;
    if (vn.getHours() === 2 && vn.getMinutes() === 0 && __lastDriveBackupDay !== day) {
      __lastDriveBackupDay = day;
      backupDbToDrive('NIGHTLY').catch(() => {});
    }
  } catch (e) {}
}, 60 * 1000);

// API Admin backup ngay + xem danh sách bản backup
app.post('/api/admin/drive/backup', authMiddleware, roleCheck(['Admin']), async (req, res) => {
  const r = await backupDbToDrive('MANUAL:' + (req.user.username || 'admin'));
  audit(req.user.username, 'DRIVE_BACKUP', 'SYSTEM', null, { ok: r.ok, file: r.name || r.skipped }, req.ip);
  if (r.ok) return res.json({ success: true, fileId: r.fileId, name: r.name, size: r.size, url: r.url });
  return res.json({ success: false, skipped: r.skipped, reason: r.reason });
});
app.get('/api/admin/drive/backups', authMiddleware, roleCheck(['Admin']), (req, res) => {
  const arr = (db.driveFiles || []).filter((f) => f.type === 'DB_BACKUP').slice(0, 30);
  res.json({ backups: arr, count: arr.length });
});
...
function applyAcceptedSwapSchedules(r, actor){
  ...
  function ensureSwapWeek(employeeIdSW, dateStr, empShift){
    ...
    if(!sc){
      ...
      addSyncQueue('SCHEDULE','CREATE',sc,actor,'WEB_HR');
    }
    return sc;
  }
  ...
  [[requester, 0], [target, 1]].forEach(([e, idx])=>{
    ...
    if(day){
      if(idx===0){
        ...
        if(isDoubleShift){
          ...
          day.shift = targetShift;
          day.secondShift = requesterShift;
    ...
    addSyncQueue('SCHEDULE','UPDATE',sched,actor,'WEB_HR');
  });
  return { isDoubleShift, targetId };
}
app.post('/api/shift-swap/:id/respond', optionalEmployeeAuth, (req,res)=>{
  ...
  if(action==='REJECT'){
    if(r.status==='PENDING_TARGET'){
      r.status='CANCELLED';
      r.rejectedBy=employeeId; r.rejectedAt=getVietnamISOString(); r.version=(r.version||1)+1;
      audit(employeeId,'REJECT_SHIFT_SWAP_TH1','SHIFT_SWAP',null,r, req.ip);
      addSyncQueue('SHIFT_SWAP','UPDATE',r, employeeId, 'WEB_EMPLOYEE');
      saveDB();
      const notifCancel = { id: uuidv4(), to: r.requesterId, type:'SHIFT_SWAP_CANCELLED', title:`${emp.name} đã từ chối đổi ca ${r.date}`, content:`${emp.name} không đồng ý đổi ca ngày ${r.date}. Phiếu đã hủy - bạn có thể tạo yêu cầu mới.`, createdAt: getVietnamISOString(), read:false, requestId: r.id };
      db.notifications.unshift(notifCancel);
      saveDB();
      io.emit('shiftSwap:update', db.shiftSwapRequests);
      io.emit('shiftSwapRequests:update', db.shiftSwapRequests);
      io.emit('notifications:update', db.notifications);
      return res.json({success:true, request:r, next:'CANCELLED'});
    } else {
  ...
  if(action==='ACCEPT'){
    if(r.status==='PENDING_TARGET'){
      ...
      io.emit('shiftSwap:update', db.shiftSwapRequests);
      io.emit('shiftSwapRequests:update', db.shiftSwapRequests);
      const doubleShiftText = isDoubleShift ? ` (LÀM 2 CA: ${getShiftVi(targetShift)} + ${getShiftVi(requesterShift)})` : '';
      const doubleShiftWarn = isDoubleShift ? ` Cảnh báo: ${target ? target.name : r.targetEmployeeName} sẽ làm 2 ca trong ngày.` : '';
      const notif1 = { id: uuidv4(), to: r.requesterId, type:'SHIFT_SWAP_ACCEPTED', title:`${emp.name} đã đồng ý đổi ca ${r.date}`, content:`${emp.name} đã đồng ý đổi ca ngày ${r.date}${doubleShiftText}. Phiếu đang chờ HR/Admin duyệt.`, createdAt: getVietnamISOString(), read:false };
      db.notifications.unshift(notif1);
      notifyAdminAndHR({
        action: 'shift_swap_accepted',
        employeeId: emp.employeeId,
        employeeName: emp.name,
        branchId: emp.branchId,
        title: `Đổi ca chờ duyệt: ${r.requesterName} <-> ${target ? target.name : (r.targetEmployeeName || '')}`,
        message: `${emp.name} đã đồng ý đổi ca với ${r.requesterName} ngày ${fmtDMY(r.date)}${doubleShiftText}.${doubleShiftWarn} Vui lòng duyệt/từ chối.`,
        type: 'success',
        data: { requestId: r.id, requesterId: r.requesterId, targetEmployeeId: r.targetEmployeeId, doubleShift: isDoubleShift }
      });
      io.emit('notifications:update', db.notifications);
      audit(employeeId,'ACCEPT_SHIFT_SWAP_TH1','SHIFT_SWAP',null,r, req.ip);
      addSyncQueue('SHIFT_SWAP','UPDATE',r, employeeId, 'WEB_EMPLOYEE');
      saveDB();
      return res.json({success:true, request:r, message: 'Đồng nghiệp đã đồng ý - phiếu đang chờ HR/Admin duyệt'});
...
app.post('/api/off-work-swap', optionalEmployeeAuth, (req,res)=>{
  const requesterId = req.body.requesterId || req.body.employeeId || req.user?.employeeId;
  const { offDate, workDate, reason, swapCode, workShift, date, toType } = req.body;
  const emp = db.employees.find(e=>e.employeeId===requesterId);
  if(!emp) return res.status(404).json({error:'Không tìm thấy nhân viên yêu cầu'});
  ...
  const now = getVietnamNow();
  const cleanWorkShift = ['CA_SANG','CA_CHIEU','CA_TOI'].includes(workShift) ? workShift : null;
  const isSingle = !!date || (!!toType && (!offDate || !workDate));

  let newReq;
  if(isSingle){
    const targetDate = date || offDate || workDate;
    const targetToType = toType || (offDate ? 'OFF' : 'WORKING');
    if(!targetDate) return res.status(400).json({error:'Vui lòng chọn ngày cần đổi'});
    if(targetToType==='WORKING' && !cleanWorkShift) return res.status(400).json({error:'Vui lòng chọn ca khi thành ngày làm (CA_SANG, CA_CHIEU, CA_TOI)'});
    const sched = db.schedules.find(s=>s.employeeId===requesterId && (s.days||[]).some(d=>d.date===targetDate));
    if(!sched) return res.status(404).json({error:`Không tìm thấy lịch ngày ${targetDate}`});
    const day = sched.days.find(d=>d.date===targetDate);
    if(!day) return res.status(404).json({error:`Không tìm thấy ngày ${targetDate} trong lịch`});
    if(targetToType==='OFF' && day.status==='OFF') return res.status(400).json({error:`Ngày ${targetDate} hiện đã là ngày nghỉ OFF`});
    if(targetToType==='WORKING' && day.status==='WORKING' && (!cleanWorkShift || day.shift===cleanWorkShift)) return res.status(400).json({error:`Ngày ${targetDate} hiện đã là ca ${day.shift}`});
    const dup = (db.shiftSwapRequests||[]).find(r=>r.type==='OFF_WORK_SWAP' && r.requesterId===requesterId && r.status.includes('PENDING') && (r.date===targetDate || r.offDate===targetDate || r.workDate===targetDate));
    if(dup) return res.status(409).json({error:'Đã có yêu cầu đổi cho ngày này đang chờ duyệt', request: dup});
    newReq = {
      id: uuidv4(), type: 'OFF_WORK_SWAP',
      requesterId, requesterName: emp.name, branchId: emp.branchId,
      date: targetDate, toType: targetToType, workShift: cleanWorkShift, reason: String(reason).trim(),
      status: 'PENDING',
    ...
    io.emit('shiftSwap:update', db.shiftSwapRequests);
    io.emit('shiftSwapRequests:update', db.shiftSwapRequests);
    const toTypeText = targetToType==='OFF'?'Nghỉ (OFF)':`Làm (${getShiftVi(cleanWorkShift)})`;
    notifyAdminAndHR({
      ...
      message: `${emp.name} (${requesterId}) xin đổi ngày ${fmtDMY(targetDate)} sang ${toTypeText}. Lý do: ${reason}`,
      type: 'info', data: { requestId: newReq.id, date: targetDate, toType: targetToType, workShift: cleanWorkShift }
    });
    io.emit('notifications:update', db.notifications);
    return res.json({success:true, request:newReq, message:'Đã gửi thẳng Admin/HR duyệt'});
  }

  // 2-day swap (cặp ngày offDate ↔ workDate)
  ...
  newReq = {
    id: uuidv4(), type: 'OFF_WORK_SWAP',
    requesterId, requesterName: emp.name, branchId: emp.branchId,
    offDate, workDate, workShift: cleanWorkShift, reason: String(reason).trim(),
    status: 'PENDING',
    createdAt: now.toISOString(), version: 1,
    isTest: (emp.isTest || isTestRecord(emp) || req.headers['x-is-test']==='true' || (req.body && req.body.isTest===true)) || undefined
  };
...
app.post('/api/off-work-swap/:id/approve', authMiddleware, roleCheck(['Admin','HR','Manager']), (req,res)=>{
  ...
  if(r.date && r.toType){
    captureSwapSnapshot(r, r.requesterId, r.date);
    const isToOff = r.toType === 'OFF';
    const f = flipOffWorkDay(r.requesterId, r.date, isToOff, isToOff ? null : (r.workShift || (emp?emp.shift:null)));
    if(!f) return res.status(404).json({error:'Không tìm thấy lịch chứa ngày này'});
    r.status='APPROVED'; r.approvedBy=req.user.username; r.approvedAt=getVietnamISOString(); r.version=(r.version||1)+1;
    ...
    addSyncQueue('SCHEDULE','UPDATE',f.sched,req.user.username,'WEB_HR');
    saveDB();
    io.emit('shiftSwap:update', db.shiftSwapRequests);
    io.emit('shiftSwapRequests:update', db.shiftSwapRequests);
    io.emit('schedules:update', db.schedules);
    const changeDesc = isToOff ? 'thành ngày nghỉ OFF' : `thành ngày làm ca ${getShiftVi(f.day.shift)}`;
    db.notifications.unshift({ id: uuidv4(), to: r.requesterId, type:'OFF_WORK_SWAP_APPROVED', title:'Đổi OFF ↔ ca làm đã duyệt', content:`Ngày ${fmtDMY(r.date)} đã được ${req.user.username} duyệt chuyển ${changeDesc}. Lịch đã cập nhật.`, createdAt: getVietnamISOString(), read:false });
    io.emit('notifications:update', db.notifications);
    return res.json({success:true, request:r, message:`Đã duyệt: ngày ${fmtDMY(r.date)} ${changeDesc}`});
  }
  ...
  const f1 = flipOffWorkDay(r.requesterId, r.offDate, false, r.workShift || (emp?emp.shift:null));
  ...
  io.emit('shiftSwapRequests:update', db.shiftSwapRequests);
  io.emit('schedules:update', db.schedules);
  db.notifications.unshift({ id: uuidv4(), to: r.requesterId, type:'OFF_WORK_SWAP_APPROVED', title:`Đổi OFF <-> ca làm đã duyệt`, content:`${r.offDate} (OFF) <-> ${r.workDate} (làm) đã được ${req.user.username} duyệt. Lịch đã cập nhật.`, createdAt: getVietnamISOString(), read:false });
  io.emit('notifications:update', db.notifications);
...
app.post('/api/off-work-swap/:id/reject', authMiddleware, roleCheck(['Admin','HR','Manager']), (req,res)=>{
  ...
  io.emit('shiftSwap:update', db.shiftSwapRequests);
  io.emit('shiftSwapRequests:update', db.shiftSwapRequests);
  const targetTxt = r.date ? `ngày ${r.date}` : `${r.offDate} <-> ${r.workDate}`;
  db.notifications.unshift({ id: uuidv4(), to: r.requesterId, type:'OFF_WORK_SWAP_REJECTED', title:`Đổi OFF <-> ca làm bị từ chối`, content:`Yêu cầu đổi ${targetTxt} đã bị ${req.user.username} từ chối.`, createdAt: getVietnamISOString(), read:false });
  io.emit('notifications:update', db.notifications);
...
app.post('/api/off-work-swap/:id/revoke', authMiddleware, roleCheck(['Admin','HR','Manager']), (req,res)=>{
  ...
  if(r.status==='APPROVED'){
    if(r.date && r.toType){
      ...
      const bDate = before.find(b=>b.employeeId===r.requesterId && b.day && b.day.date===r.date);
      if(bDate && bDate.day){
        const s = restoreSwapSnapshotDay(r.requesterId, bDate.day);
        if(s){ addSyncQueue('SCHEDULE','UPDATE',s,req.user.username,'WEB_HR'); flipped=1; }
      } else {
        const wasToOff = r.toType === 'OFF';
        const f = flipOffWorkDay(r.requesterId, r.date, !wasToOff, wasToOff ? (emp?emp.shift:null) : null);
        if(f){ addSyncQueue('SCHEDULE','UPDATE',f.sched,req.user.username,'WEB_HR'); flipped=1; }
      }
    } else {
      const before = Array.isArray(r.beforeSchedule) ? r.beforeSchedule : [];
  ...
  audit(req.user.username,'REVOKE_OFF_WORK_SWAP','SHIFT_SWAP',{date:r.date, offDate:r.offDate, workDate:r.workDate},{revoked:true, flipped},req.ip);
  addSyncQueue('SHIFT_SWAP','UPDATE',r,req.user.username,'WEB_HR');
  saveDB();
  io.emit('shiftSwap:update', db.shiftSwapRequests);
  io.emit('shiftSwapRequests:update', db.shiftSwapRequests);
  io.emit('schedules:update', db.schedules);
  const targetDesc = r.date ? `ngày ${r.date}` : `${r.offDate} <-> ${r.workDate}`;
  db.notifications.push({ id: uuidv4(), to: r.requesterId, type:'OFF_WORK_SWAP_REVOKED', title:`Đổi OFF <-> ca làm bị thu hồi`, content:`Phiếu đổi ${targetDesc} đã bị ${req.user.username} thu hồi.${flipped?` Lịch đã hoàn về như trước khi duyệt.`:''}`, createdAt: getVietnamISOString(), read:false });
  io.emit('notifications:update', db.notifications);
  res.json({success:true, request:r, revertedDays:flipped, message: flipped?`Đã thu hồi: ${targetDesc} về như trước khi duyệt`:'Đã thu hồi phiếu đang chờ'});
});
...
app.post('/api/shift-swap/:id/approve', authMiddleware, roleCheck(['Admin','HR','Manager']), (req,res)=>{
  const r = db.shiftSwapRequests.find(x=>x.id===req.params.id);
  if(!r) return res.status(404).json({error:'Không tìm thấy yêu cầu'});
  if(r.status==='APPROVED' || r.status==='REJECTED' || r.status==='AUTO_APPROVED' || r.status==='CANCELLED') return res.status(400).json({error:'Yêu cầu đã xử lý'});

  const wasPendingHR = (r.status==='PENDING_HR');
  r.status='APPROVED';
  ...
  if(wasPendingHR){
    applyAcceptedSwapSchedules(r, req.user.username);
    if(r.acceptedBy) addShiftAssistanceAllowance(r, r.acceptedBy);
  } else {
  ...
  if(targetId){
    ...
    db.notifications.unshift(notif2);
  }
  notifyAdminAndHR({
...
app.post('/api/shift-swap/:id/cancel', optionalEmployeeAuth, (req,res)=>{
  const employeeId = req.body.employeeId || req.user?.employeeId;
  const r = db.shiftSwapRequests.find(x=>x.id===req.params.id);
  if(!r) return res.status(404).json({error:'Không tìm thấy yêu cầu'});
  if(!employeeId || r.requesterId!==employeeId) return res.status(403).json({error:'Chỉ người tạo mới được hủy phiếu này'});
  if(req.user?.employeeId && req.user.employeeId!==employeeId) return res.status(403).json({error:'Không được hủy thay nhân viên khác'});
  const cancellable = ['PENDING_TARGET','PENDING_BROADCAST','PENDING_BROADCAST_ACCEPTED','PENDING','PENDING_HR'];
  if(!cancellable.includes(r.status)) return res.status(400).json({error:'Chỉ hủy được phiếu đang chờ xử lý'});
  r.status='CANCELLED';
  r.cancelledBy=employeeId; r.cancelledAt=getVietnamISOString(); r.version=(r.version||1)+1;
  audit(employeeId,'CANCEL_SHIFT_SWAP','SHIFT_SWAP',null,r, req.ip);
  addSyncQueue('SHIFT_SWAP','UPDATE',r, employeeId, 'WEB_EMPLOYEE');
  saveDB();
  const otherId = r.acceptedBy || r.targetEmployeeId;
  if(otherId && otherId!==employeeId){
    const notif = { id: uuidv4(), to: otherId, type:'SHIFT_SWAP_CANCELLED', title:`${r.requesterName} đã hủy yêu cầu đổi ca ${r.date||r.offDate||''}`, content:`Phiếu đổi ca đã bị người tạo hủy.`, createdAt: getVietnamISOString(), read:false, requestId: r.id };
    db.notifications.unshift(notif);
    saveDB();
  }
  io.emit('shiftSwap:update', db.shiftSwapRequests);
  io.emit('shiftSwapRequests:update', db.shiftSwapRequests);
  io.emit('notifications:update', db.notifications);
  res.json({success:true, request:r, message:'Đã hủy phiếu - bạn có thể tạo yêu cầu mới'});
});
...
app.post('/api/shift-swap/:id/revoke', authMiddleware, roleCheck(['Admin','HR','Manager']), (req,res)=>{
  ...
  if(r.status==='REVOKED' || r.status==='REJECTED' || r.status==='EXPIRED' || r.status==='CANCELLED') return res.status(400).json({error:'Phiếu đã kết thúc, không thể thu hồi'});
  const requester = db.employees.find(e=>e.employeeId===r.requesterId);
...
function broadcastTelegramScheduleApproved(drafts, weekStart) {
  try {
    const cfgNv = getTelegramCfg('employee');
    if (!cfgNv.botToken || OUTBOUND_SYNC_DISABLED) return;
    for (const d of (drafts || [])) {
      const link = (db.telegramLinks || []).find(l => l.employeeId === d.employeeId && l.chatId);
      if (!link) continue;
      const emp = (db.employees || []).find(e => e.employeeId === d.employeeId);
      const lines = (d.days || []).map(day => {
        const isOff = day.status === 'OFF' || day.shift === 'OFF';
        return `• <b>${fmtDMY(day.date)} (${day.dayName || ''})</b>: ${day.shift || 'Ca làm'} ${isOff ? '🏖️ (NGHỈ OFF)' : '💼 (LÀM VIỆC)'}`;
      });
      const msg = `🎉 <b>LỊCH LÀM VIỆC TUẦN SAU ĐÃ ĐƯỢC HR PHÊ DUYỆT!</b>\n\n`
        + `👤 <b>${emp ? emp.name : d.employeeId}</b> (<code>${d.employeeId}</code>)\n`
        + `📅 Tuần bắt đầu: <b>${fmtDMY(weekStart)}</b>\n\n`
        + lines.join('\n')
        + `\n\n📌 <i>Lịch làm việc đã chính thức có hiệu lực. Nếu cần đổi ca, bạn vui lòng sử dụng tính năng <b>/doica</b> nhé!</i>`;
      tg.sendTelegramMessage(cfgNv.botToken, link.chatId, msg).catch(() => {});
    }
  } catch (e) {
    console.error('[BROADCAST_SCHEDULE_APPROVED] Error:', e.message);
  }
}
...
app.post('/api/schedules/approve-next-week', authMiddleware, roleCheck(['Admin','HR']), (req,res)=>{
  ...
  // Tự động bắn thông báo lịch tuần sau tới BOT Telegram của từng nhân viên
  broadcastTelegramScheduleApproved(drafts, nextWeekStart);
...
app.post('/api/off-requests', (req,res)=>{
  ...
  if(isOfficial){
    ...
    const isTestEmp = emp.isTest || isTestRecord(emp) || req.headers['x-is-test'] === 'true';
    ...
    for(const date of dates){
      const conflict = checkOffConflict(emp.branchId, emp.shift, date, employeeId, isTestEmp);
      ...
      const schedHit = db.schedules.find(s=>{
        ...
        if(isTestEmp && !o.isTest && !isTestRecord(o)) return false; // Test requester doesn't conflict with real employees
        if(!isTestEmp && (o.isTest || isTestRecord(o))) return false; // Real requester doesn't conflict with test employees
...
app.post('/api/emergency-requests/:id/respond', (req,res)=>{
  const { substituteId, action, doubleShift } = req.body; // action APPROVE/REJECT, doubleShift: boolean
  ...
  let sched2 = ensureWeekSchedule(substituteId, er.date, substituteShift);
  {
    ...
    if(day){
      if(isDoubleShift){
        ...
        // Normal substitute: Employee B takes Employee A's shift
        day.status = 'SUBSTITUTE';
        day.shift = requesterShift;
        day.substituteFor = er.employeeId;
      }
  ...
  io.emit('emergencyRequests:update', db.emergencyRequests);
  io.emit('schedules:update', db.schedules);
  const doubleShiftText = isDoubleShift ? ` (LÀM 2 CA: ${getShiftVi(substituteShift)} + ${getShiftVi(requesterShift)})` : '';
  notifyAdminAndHR({
    ...
    message: `${subEmp.name} đã chấp nhận thay ca ngày ${fmtDMY(er.date)} cho ${er.employeeName}${doubleShiftText}. Lịch đã được cập nhật.`,
    type: 'success',
    data: { requestId: er.id, substituteId, originalEmployeeId: er.employeeId, doubleShift: isDoubleShift }
  });
  const zr = { id: uuidv4(), sent_at: getVietnamISOString(), receiver: db.employees.find(e=>e.employeeId===er.employeeId)?.phone, type:'EMERGENCY_APPROVED', content:`OFF CA LÀM ngày ${er.date} đã được duyệt, người thay: ${subEmp.name}${doubleShiftText}`, status:'SENT', error:'' };
  db.zaloRecords.unshift(zr);
  io.emit('zalo:update', db.zaloRecords);
  res.json({ ...er, doubleShift: isDoubleShift });
});
...
// API Admin nạp lại DB từ đĩa (dùng sau khi scp db.json lên server — khỏi restart).
// An toàn: validate file trước, backup bản RAM ra db_before_reload.json, lỗi thì giữ nguyên RAM.
app.post('/api/admin/db/reload', authMiddleware, roleCheck(['Admin']), (req,res)=>{
  try{
    if(!fs.existsSync(DATA_FILE)) return res.status(404).json({ error:'Không thấy file data/db.json trên server' });
    let parsed;
    try{ parsed = JSON.parse(fs.readFileSync(DATA_FILE,'utf8')); }
    catch(e){ return res.status(400).json({ error:'File db.json lỗi cú pháp — giữ nguyên dữ liệu trong RAM', detail: e.message }); }
    if(!parsed || typeof parsed!=='object' || !Array.isArray(parsed.employees) || !Array.isArray(parsed.keys)){
      return res.status(400).json({ error:'File db.json không đúng cấu trúc (thiếu employees/keys) — giữ nguyên dữ liệu trong RAM' });
    }
    try{ fs.writeFileSync(path.join(path.dirname(DATA_FILE),'db_before_reload.json'), JSON.stringify(db)); }catch(e){}
    loadDB();
    audit(req.user.username,'DB_RELOAD','SYSTEM',null,{employees:db.employees.length, keys:db.keys.length}, req.ip);
    try{
    ...
    res.json({ success:true, employees:db.employees.length, keys:(db.keys||[]).length, applicants:(db.applicants||[]).length, attendances:(db.attendances||[]).length, at:getVietnamISOString() });
  }catch(e){ res.status(500).json({ error:'Nạp DB thất bại — giữ nguyên dữ liệu trong RAM', detail: e.message }); }
});
...
function logTelegram(direction, chatId, text, status){
  ...
  try{
    ...
    if(db.telegramLogs.length>200) db.telegramLogs.length = 200;
  }catch(e){}
}
...
function notifyTelegramForPhone(phone, text){
  try{
    ...
    if(!link) return;
    tg.sendTelegramMessage(cfg.botToken, link.chatId, text).then(r=>{
      logTelegram('OUT', link.chatId, text, r.ok?'SENT':'FAILED');
    }).catch(()=>{});
  }catch(e){}
}
...
async function notifyHRMaster(kind, text, probe, extra = {}) {
  try {
    ...
    if (!db.sentAdminNotifKeys) db.sentAdminNotifKeys = [];
    ...
    if (!cfg.botToken) {
      db.adminNotificationQueue.push({
        ...
        kind, text, extra, dedupeKey,
        createdAt: getVietnamISOString()
      });
      if (db.adminNotificationQueue.length > 500) db.adminNotificationQueue = db.adminNotificationQueue.slice(-500);
      if (dedupeKey) {
        db.sentAdminNotifKeys.push(dedupeKey);
        if (db.sentAdminNotifKeys.length > 1000) db.sentAdminNotifKeys = db.sentAdminNotifKeys.slice(-1000);
      }
    ...
    const now = Date.now();
    const activeHrChats = (db.hrBotChats || []).filter((c) => {
    ...
    if (!activeHrChats.length) {
      // Zero Miss: lưu hàng đợi thông báo cho HR khi offline/chưa đăng nhập
      db.adminNotificationQueue.push({
        id: 'notif_' + Date.now() + '_' + Math.random().toString(36).substr(2, 5),
        kind, text, extra, dedupeKey,
        createdAt: getVietnamISOString()
  ...
  if (!username || !oldPass || !newPass) {
    return {
      text: '🔑 <b>CÚ PHÁP ĐỔI MẬT KHẨU TÀI KHOẢN:</b>\n\n'
        ...
        + '📌 <i>Lưu ý bảo mật: Tuyệt đối không chia sẻ mật khẩu cho người khác.</i>'
    };
  }
...
async function processTelegramUpdate(update, role){
  ...
  const ctx = {
    ...
    hrLogin: async (chatId, username, password) => {
      ...
      // Drain queue và lọc bỏ thông báo trùng lặp (Zero Duplicate & Zero Miss)
      const seenKeys = new Set();
      const uniquePending = [];
      for (const notif of db.adminNotificationQueue) {
        const k = notif.dedupeKey || notif.id;
        if (!seenKeys.has(k)) {
          seenKeys.add(k);
          uniquePending.push(notif);
        }
      }
      db.adminNotificationQueue = [];
      saveDB();
      return { ok:true, user: c.auth, pendingNotifications: uniquePending };
    },
    ...
    adminResetSheet: async (session, sheetName) => {
      ...
      try {
        ...
        if (token && spreadsheetId && !OUTBOUND_SYNC_DISABLED) {
          const clearUrl = `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${encodeURIComponent(cleanName)}!A2:Z5000:clear`;
          await fetch(clearUrl, { method: 'POST', headers: { Authorization: `Bearer ${token}` } });
        }
      } catch (e) {
        console.error('[ADMIN_RESET_SHEET] Clear sheet error:', e.message);
      }
    ...
    hrUpdateEmployeeInfo: async (session, rawArgs) => {
      ...
      if (firstSpaceIdx === -1) {
        return { text: '⚠️ Thiếu thông tin cần sửa. Cú pháp: <code>/sua_thongtin_nhanvien: &lt;Mã_NV&gt; &lt;ca_cũ&gt; sang &lt;ca_mới&gt;, &lt;CN_cũ&gt; sang &lt;CN_mới&gt;</code>' };
      }
      const empCode = trimmed.slice(0, firstSpaceIdx).replace(/[,:]/g, '').trim();
      const changesPart = trimmed.slice(firstSpaceIdx).trim();
      ...
      let newShift = null;
      let newBranch = null;
      const clauses = changesPart.split(/[,;\n]+/);
      for (const clause of clauses) {
        const m = clause.split(/\s+sang\s+/i);
        if (m.length === 2) {
          const left = m[0].trim().toLowerCase();
          const right = m[1].trim().toLowerCase();
          if (left.includes('ca') || ['sang', 'chieu', 'toi', 'trua', 'off'].some(s => left.includes(s))) {
            newShift = parseShiftCode(right);
          } else if (left.includes('cn') || ['cn1', 'cn2', 'cn3', 'cn4'].some(c => left.includes(c))) {
            const bFound = (db.branches || []).find(b => b.id.toLowerCase() === right.replace(/\s+/g, '') || right.includes(b.id.toLowerCase()));
            if (bFound) newBranch = bFound.id;
            else {
              const upper = right.toUpperCase().replace(/\s+/g, '');
              if (['CN1', 'CN2', 'CN3', 'CN4'].includes(upper)) newBranch = upper;
            }
          } else {
            const testShift = parseShiftCode(right);
            if (right.includes('sang') || right.includes('chieu') || right.includes('toi')) {
              newShift = testShift;
            } else {
              const upper = right.toUpperCase().replace(/\s+/g, '');
              if (['CN1', 'CN2', 'CN3', 'CN4'].includes(upper)) newBranch = upper;
      ...
      if (!newShift && !newBranch) {
        return { text: '⚠️ Không nhận diện được thông tin ca làm hoặc chi nhánh mới cần đổi. Vui lòng kiểm tra cú pháp <code>&lt;cũ&gt; sang &lt;mới&gt;</code>.' };
      }

      const updateLogs = [];
      if (newShift && newShift !== emp.shift) {
        updateLogs.push(`• Ca làm: <code>${emp.shift || 'Chưa gán'}</code> ➔ <b>${newShift}</b>`);
        emp.shift = newShift;
      }
      if (newBranch && newBranch !== emp.branchId) {
        updateLogs.push(`• Chi nhánh: <code>${emp.branchId || 'Chưa gán'}</code> ➔ <b>${newBranch}</b>`);
        emp.branchId = newBranch;
      }

      if (updateLogs.length === 0) {
        return { text: `ℹ️ Thông tin của nhân viên <b>${emp.name}</b> (${emp.employeeId}) đã trùng khớp với yêu cầu, không cần cập nhật.` };
      }

      emp.version = (emp.version || 1) + 1;
      emp.updated_at = getVietnamISOString();
      emp.updated_by = session.username || 'HR_BOT';
      emp.sync_status = 'SYNCED';
      ...
      try {
        addSyncQueue('EMPLOYEE', 'UPDATE', emp, session.username, 'TELEGRAM_BOT');
        if (typeof triggerRealtimeSheetSync === 'function') {
      ...
      try {
        audit(session.username, 'UPDATE_EMPLOYEE_INFO', 'EMPLOYEE', emp.employeeId, { newShift, newBranch }, 'telegram_bot');
      } catch (e) {}
    ...
    linkEmployee: async (telegramId, username, employeeId, key, chatId)=>{
      ...
      const keyRec = (db.keys||[]).find(k=>k.employeeId===employeeId && k.key===key);
      if(!keyRec) return { ok:false, error:'KEY không hợp lệ.' };
      if(keyRec.status!=='ACTIVE') return { ok:false, error:'KEY đã bị vô hiệu hóa.' };
      let link = findTelegramLink(telegramId);
      if(!link){ link = { telegramId: String(telegramId), username: username||'', chatId: chatId||telegramId, createdAt: getVietnamISOString() }; db.telegramLinks.push(link); }
      link.employeeId = employeeId; link.chatId = chatId||telegramId; link.username = username||link.username; link.linkedAt = getVietnamISOString();
      saveDB();
      try{ io.emit('telegram:update', { count: db.telegramLinks.length }); }catch(e){}
      return { ok:true, label: `${emp.name} (${employeeId})` };
    },
    linkByStartPayload: async (telegramId, payload)=>{
      // payload dạng EMP:<employeeId>:<key> (tạo từ Mini App nút "Liên kết Telegram")
      const m = String(payload||'').match(/^EMP:(.+):(.+)$/);
      if(!m) return { ok:false, error:'Mã liên kết không hợp lệ.' };
      const emp = (db.employees||[]).find(e=>e.employeeId===m[1]);
      if(!emp) return { ok:false, error:'Mã nhân viên không tồn tại.' };
      const keyRec = (db.keys||[]).find(k=>k.employeeId===m[1] && k.key===m[2]);
      if(!keyRec) return { ok:false, error:'KEY không hợp lệ.' };
      let link = findTelegramLink(telegramId);
      if(!link){ link = { telegramId: String(telegramId), createdAt: getVietnamISOString() }; db.telegramLinks.push(link); }
      link.employeeId = m[1]; link.linkedAt = getVietnamISOString();
      saveDB();
      try{ io.emit('telegram:update', { count: db.telegramLinks.length }); }catch(e){}
    ...
    getScheduleWithNextWeekStatus: async (telegramId) => {
      const link = findTelegramLink(telegramId);
      if (!link?.employeeId) return { text: '⚠️ <b>Bạn chưa liên kết tài khoản</b>\nVui lòng nhắn số điện thoại của bạn (hoặc /link <code>SĐT</code>) vào đây để liên kết nhé.' };
      const emp = (db.employees || []).find(e => e.employeeId === link.employeeId);
      if (!emp) return { text: 'Tài khoản liên kết không còn tồn tại.' };
      ...
      const todayStr = getVietnamTodayStr();
      const curMon = toVietnamDateStr(getMonday(new Date(todayStr)));
      const nextMon = toVietnamDateStr(getNextMonday());

      const curSchedule = (db.schedules || []).find(s => s.employeeId === emp.employeeId && s.weekStart === curMon);
      let curLines = '';
      if (curSchedule && curSchedule.days && curSchedule.days.length > 0) {
        curLines = curSchedule.days.map(d => {
          const isOff = d.status === 'OFF' || d.shift === 'OFF';
          return `• <b>${fmtDMY(d.date)} (${d.dayName || ''})</b>: ${d.shift || 'Ca làm'} ${isOff ? '🏖️ (NGHỈ OFF)' : '💼 (LÀM VIỆC)'}`;
        }).join('\n');
      } else {
        const upcoming = (db.schedules || []).filter(s => s.employeeId === emp.employeeId && s.date >= todayStr).slice(0, 7);
        if (upcoming.length > 0) {
          curLines = upcoming.map(s => {
            const isOff = s.status === 'OFF' || s.shift === 'OFF';
            return `• <b>${fmtDMY(s.date)}</b>: ${s.shift || 'Ca làm'} ${isOff ? '🏖️ (NGHỈ OFF)' : '💼 (LÀM VIỆC)'}`;
          }).join('\n');
        } else {
          curLines = '• Chưa có lịch xếp sẵn cho tuần này.';
        }
      }

      const nextSchedule = (db.schedules || []).find(s => s.employeeId === emp.employeeId && s.weekStart === nextMon);
      let nextWeekText = '';
      if (nextSchedule && nextSchedule.approvalStatus === 'APPROVED' && nextSchedule.days) {
        const nextLines = nextSchedule.days.map(d => {
          const isOff = d.status === 'OFF' || d.shift === 'OFF';
          return `• <b>${fmtDMY(d.date)} (${d.dayName || ''})</b>: ${d.shift || 'Ca làm'} ${isOff ? '🏖️ (NGHỈ OFF)' : '💼 (LÀM VIỆC)'}`;
        }).join('\n');
        nextWeekText = `\n\n🎉 <b>LỊCH LÀM VIỆC TUẦN SAU (${fmtDMY(nextMon)}) — ĐÃ PHÊ DUYỆT:</b>\n${nextLines}`;
      } else {
        nextWeekText = `\n\n⏳ <b>LỊCH LÀM VIỆC TUẦN SAU (${fmtDMY(nextMon)}):</b>\n⚠️ <i>Lịch tuần sau của bạn HR chưa duyệt lịch và yêu cầu chờ HR duyệt lịch nhé... Khi HR duyệt, BOT telegram tự động thông báo lịch tuần sau cho nhân viên biết!</i>`;
      }
      ...
      return {
        text: `📅 <b>LỊCH LÀM VIỆC HIỆN TẠI & TUẦN TỚI</b>\n`
          ...
          + curLines
          + nextWeekText
          + `\n\n💬 <i>Nếu có nhu cầu đổi ca, bạn chỉ cần dùng lệnh <b>/doica</b> nhé!</i>`
      };
    },
    checkExistingOffRegistration: async (telegramId, dates) => {
      const link = findTelegramLink(telegramId);
      if (!link || !link.employeeId) return { hasRegistered: false };
      const emp = (db.employees || []).find(e => e.employeeId === link.employeeId);
      if (!emp) return { hasRegistered: false };

      const targetDateStr = dates && dates[0] ? dates[0] : getVietnamTodayStr();
      const targetMon = toVietnamDateStr(getMonday(new Date(targetDateStr)));

      const offReq = (db.offRequests || []).find(r => r.employeeId === emp.employeeId && r.status !== 'REJECTED' && (
        (r.dates && r.dates.some(d => toVietnamDateStr(getMonday(new Date(d))) === targetMon)) ||
        (r.weekStart && r.weekStart === targetMon)
      ));

      if (offReq && offReq.dates && offReq.dates.length >= 2) {
        return {
          hasRegistered: true,
          dates: offReq.dates.map(d => fmtDMY(d)),
          status: offReq.status
        };
      }

      const sc = (db.schedules || []).find(s => s.employeeId === emp.employeeId && s.weekStart === targetMon);
      if (sc && sc.days) {
        const offDays = sc.days.filter(d => d.status === 'OFF' || d.shift === 'OFF');
        if (offDays.length >= 2) {
          return {
            hasRegistered: true,
            dates: offDays.map(d => fmtDMY(d.date)),
    ...
    getBranchColleagues: async (telegramId) => {
      ...
      const emp = (db.employees || []).find(e => e.employeeId === link.employeeId);
      if (!emp) return { colleagues: [] };

      const colleagues = (db.employees || []).filter(e =>
        e.branchId === emp.branchId &&
        e.employeeId !== emp.employeeId &&
        !['ARCHIVED', 'TERMINATED', 'RESIGNED'].includes(e.status)
      ).slice(0, 10);

      const bObj = (db.branches || []).find(b => b.id === emp.branchId);
      const branchName = bObj ? bObj.name : emp.branchId;
    ...
    getSwapComparison: async (telegramId, colleagueId) => {
      const link = findTelegramLink(telegramId);
      if (!link?.employeeId) return { ok: false, error: 'Chưa liên kết tài khoản' };
      const empA = (db.employees || []).find(e => e.employeeId === link.employeeId);
      const empB = findEmployeeByShortOrFullId(colleagueId);
      if (!empA || !empB) return { ok: false, error: 'Không tìm thấy nhân viên' };
      if (empA.branchId !== empB.branchId) {
        return { ok: false, error: `Không cùng chi nhánh (${empA.branchId} vs ${empB.branchId})` };
      }

      const todayStr = getVietnamTodayStr();
      const curMon = toVietnamDateStr(getMonday(new Date(todayStr)));
      const scA = (db.schedules || []).find(s => s.employeeId === empA.employeeId && s.weekStart === curMon);
      const scB = (db.schedules || []).find(s => s.employeeId === empB.employeeId && s.weekStart === curMon);

      const scheduleCompareLines = [];
      if (scA?.days && scB?.days) {
        for (let i = 0; i < 7; i++) {
          const dA = scA.days[i];
          const dB = scB.days[i];
          if (dA && dB && dA.date >= todayStr) {
            scheduleCompareLines.push(`• <b>${fmtDMY(dA.date)} (${dA.dayName})</b>: Bạn [${dA.shift}] ⟷ ${empB.name} [${dB.shift}]`);
          }
        }
      }

      const comparisonText = `🔄 <b>SO SÁNH LỊCH ĐỔI CA — CÙNG CHI NHÁNH ${empA.branchId}</b>\n\n`
        + `👤 <b>Bạn:</b> ${empA.name} (<code>${empA.employeeId}</code>)\n`
        + `👤 <b>Đồng nghiệp:</b> ${empB.name} (<code>${empB.employeeId}</code>)\n\n`
        + `📅 <b>LỊCH LÀM VIỆC TRONG TUẦN ĐỐI CHIẾU:</b>\n`
        + (scheduleCompareLines.length > 0 ? scheduleCompareLines.join('\n') : `• Bạn: ${empA.shift} ⟷ ${empB.name}: ${empB.shift}`)
        + `\n\n👉 <b>Để gửi yêu cầu đổi ca, bạn gõ cú pháp:</b>\n`
        + `<code>/doica &lt;ngày_của_bạn&gt; &lt;ca_của_bạn&gt; sang ${empB.name} &lt;ngày_muốn_nhận&gt; &lt;ca_muốn_nhận&gt;</code>\n\n`
        + `<i>(Lưu ý: Có thể đổi khác ngày, khác ca làm việc, miễn là cùng chi nhánh <b>${empA.branchId}</b>!)</i>`;

      return { ok: true, empA, empB, text: comparisonText };
    },
  ...
  for(const a of actions){
    try{
      logTelegram('OUT', a.chatId, a.text, 'SENDING');
      const r = await tg.sendTelegramMessage(cfg.botToken, a.chatId, a.text, a.extra||{});
      logTelegram('OUT', a.chatId, a.text, r.ok?'SENT':'FAILED');
    }catch(e){ logTelegram('OUT', a.chatId, a.text, 'FAILED: '+e.message); }
  }
}
// Polling getUpdates (dự phòng khi không dùng webhook) — tắt khi test/thiếu token
let __tgPolling = false;
async function startTelegramPolling(){
  if(__tgPolling || OUTBOUND_SYNC_DISABLED) return;
  const cfg = getTelegramCfg();
  if(!cfg.botToken || cfg.useWebhook) return;
  if(process.env.TELEGRAM_POLLING !== 'true') return; // mặc định tắt, bật bằng TELEGRAM_POLLING=true
  __tgPolling = true;
  let offset = 0;
  console.log('[TELEGRAM] Polling getUpdates bật');
  const loop = async ()=>{
    if(!process.env.TELEGRAM_POLLING) return;
    try{
      const url = `https://api.telegram.org/bot${cfg.botToken}/getUpdates?timeout=25&offset=${offset}`;
...
app.post('/api/telegram/auth', (req,res)=>{
  ...
  if(link?.employeeId){
    const emp = (db.employees||[]).find(e=>e.employeeId===link.employeeId);
    if(emp && !['ARCHIVED','TERMINATED','RESIGNED'].includes(emp.status)){
      ...
      return res.json({ ok:true, linked:true, token, employee: emp, telegramUser: v.user });
    }
  }
...
app.post('/api/telegram/send', authMiddleware, async (req,res)=>{
  ...
  if(!target && employeeId){
    const link = (db.telegramLinks||[]).find(l=> l.employeeId===employeeId);
    target = link?.chatId;
  }
  if(!target || !text) return res.status(400).json({ error:'Thiếu người nhận hoặc nội dung' });
  const r = await tg.sendTelegramMessage(cfg.botToken, target, text);
  logTelegram('OUT', target, text, r.ok?'SENT':'FAILED');
  saveDB();
  res.json({ ok: r.ok, disabled: !!r.disabled, error: r.error||null });
});
// Danh sách liên kết (Admin/HR)
